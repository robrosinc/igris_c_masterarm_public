...
DDS discovery needs a CycloneDDS XML that names the network interface reaching
the robot. A sample **`cyclonedds.xml`** ships at the root of this package; edit
its `<NetworkInterface name="...">` to the interface on this machine (`ip link`),
then point CycloneDDS at it before launching:

```bash
export CYCLONEDDS_URI=file://$PWD/cyclonedds.xml
```

This file configures ONLY the network interface. The DDS **domain id** and
**namespace** are set separately via the `domain_id:=` / `namespace:=` launch
args (see below), NOT in this XML. The development PC and the robot must use the
same domain id + namespace.
