...
int main(int argc, char **argv) {
    ...
    // Pick an initial window size based on the primary monitor — shrink on
    // small laptop displays and scale up on large desktops while keeping a
    // usable lower bound. The runtime UI scale (DisplaySize / kBase*) handles
    // proportional sizing of widgets afterwards.
    int initial_width  = static_cast<int>(kBaseWindowWidth);
    int initial_height = static_cast<int>(kBaseWindowHeight);
    ...
    if (GLFWmonitor *primary = glfwGetPrimaryMonitor()) {
        ...
        if (mode != nullptr && monitor_w > 0 && monitor_h > 0) {
            constexpr float kMonitorFraction = 0.8f;
            constexpr int   kMinWidth  = 1280;
            constexpr int   kMinHeight = 720;
            initial_width  = std::max(kMinWidth,
                                      static_cast<int>(monitor_w * kMonitorFraction));
            initial_height = std::max(kMinHeight,
                                      static_cast<int>(monitor_h * kMonitorFraction));
        }
    }

    GLFWwindow *window =
        glfwCreateWindow(initial_width, initial_height, "IGRIS-C MasterArm GUI Client", nullptr, nullptr);
    if (window == nullptr) {
    ...
    if (monitor_w > 0 && monitor_h > 0) {
        glfwSetWindowPos(window,
                         monitor_x + (monitor_w - initial_width)  / 2,
                         monitor_y + (monitor_h - initial_height) / 2);
    }
    ...
    while (!glfwWindowShouldClose(window) && g_running.load()) {
        ...
        ImGui::BeginChild("RightPanel", ImVec2(right_width, top_height), true);
        {
            ...
            if (ScaledButton("BMS OFF", ImVec2(-1, button_height))) {
